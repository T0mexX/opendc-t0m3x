...
internal class FlatFly private constructor(
    ...
    val hGrid: MultiDimGrid<HostNode>,
    ...
    override val _nodesById: MutableMap<NodeId, Node<*>> =
        ( _sendNodesById + swGrid.associateBy { it!!.id }.toMutableMap() ) as MutableMap<NodeId, Node<*>>
    ...
    companion object {
        ...
        suspend operator fun invoke(
            specs: FlatFlySpecs,
        ): FlatFly = withProgressBar(task = "Building Flatten Butterfly...", max = specs.E_.toLong() + specs.V_) pb@ {
            // Implemented only using subnets.
            if (this@NetSimScope.devConfig.netConfig.subnetOpt.not())
                log.warn("'netSimConfig.devConfig.netConfig.subnetOptimization = false' detected. " +
            ...
            val hGrid = MultiDimGrid<HostNode>(MutableList(specs.n + 1) { specs.k }.also { it[it.size - 1] = specs.c })

            /**
             * Executed recursively on the [FlatFlySpecs.n] dimensions.
             * Builds all nodes.
             */
            suspend fun buildNodes(outerSubNet: IPv4Address, vararg indices: Int) {
                ...
                if (indices.size == specs.n - 1) {
                    ...
                    (0 ..< specs.k).forEach { sIdx ->
                        ...
                        (0 ..< specs.c).map {
                            HostNode(subnet = outerSubNet, nPorts = 1)
